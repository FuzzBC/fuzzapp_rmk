...
class ArduinoCompiler:
 ...
 # Use --verbose to get progress info, but filter output. Increase timeout for compilation/linking
        ...
        try:
            ...
            # _rmk_ variant of the frozen compiler.py's own comment: both
            # sketches read WiFi/OTA credentials AND the generated protocol
            # codec (protocol_opcodes.h) from the shared, gitignored/generated
            # _rmk_Shared sibling folder via #include <WiFiCredentials.h> /
            # <protocol_opcodes.h> - a quoted relative include does NOT
            # reliably resolve under arduino-cli (confirmed on the frozen
            # side: "No such file or directory" even though the file exists
            # on disk), so it's added as an ad-hoc library search path here
            # instead. One folder covers both needs now (unlike the frozen
            # project's separate _Shared), since _rmk_Shared already existed
            # for the protocol codec and WiFiCredentials.h was consolidated
            # into it during the _rmk_~FuZz_APPv8.0 project split - see that
            # move's own notes in _rmk_docs/00_PLAN.md. Flat directory of
            # headers only (no library.properties/src/) is a valid
            # legacy-layout library.
            shared_dir = os.path.normpath(os.path.join(os.path.dirname(sketch_path), '..', '_rmk_Shared'))
